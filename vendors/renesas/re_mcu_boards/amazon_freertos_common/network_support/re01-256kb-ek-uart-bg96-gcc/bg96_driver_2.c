...
// changed 2020/10 start
    	...
    	{
// changed 2020/10 start (メモリ削減)
/////    		ercd = gsp_sci9_dev->Receive(&data_deb[0], 1600);
// changed 2020/10 start (メモリ削減)
/////    		ercd = gsp_sci9_dev->Receive(&data_deb[0], 5120); // RSA OK (4096 では溢れる場合あり)
    		ercd = gsp_sci9_dev->Receive(&data_deb[0], 6144); // RSA OK (5120 では溢れる場合あり)
// changed 2020/10 end (メモリ削減)
// changed 2020/10 end (メモリ削減)
    	...
    	new_rx_cnt = gsp_sci9_dev->GetRxCount();
...
void bg96_usart_abort(uint32_t *new_cnt, uint32_t *pre_cnt)
{
    ...
    check_new_rx_cnt = gsp_sci9_dev->GetRxCount();
    if (*new_cnt == check_new_rx_cnt)
    {
        gsp_sci9_dev->Control(ARM_USART_ABORT_RECEIVE, 0);
        *pre_cnt = 0;
        *new_cnt = 0;
        check_new_rx_cnt = 0;
    }
