...
#elif defined(__CCRL__) || defined(__ICCRL78__) || defined(__RL__)
#define WIFI_GPIO_PORT(x, y)               ( P ## x ## _bit.no ## y )

/* Reset port pin macros.  */
#define WIFI_RESET_DDR(x, y)
#define WIFI_RESET_DR(x, y)                (WIFI_GPIO_PORT(x, y))

/* RTS port pin macros.  */
#define WIFI_RTS_DDR(x, y)
#define WIFI_RTS_DR(x, y)                  (WIFI_GPIO_PORT(x, y))
#endif

/* IP address(xxx.xxx.xxx.xxx) into ULONG */
#define WIFI_IPV4BYTE_TO_ULONG(addr1, addr2, addr3, addr4) \
	(((uint32_t)addr1 & 0x000000FF) << 24) | (((uint32_t)addr2 & 0x000000FF) << 16) |\
    (((uint32_t)addr3 & 0x000000FF) << 8)  | (((uint32_t)addr4 & 0x000000FF))

/* ULONG into IP address(xxx.xxx.xxx.xxx) */
#define WIFI_ULONG_TO_IPV4BYTE_1(ul) (uint8_t)((((uint32_t)ul) >> 24) & 0x000000FF)   /* IP address byte1 */
#define WIFI_ULONG_TO_IPV4BYTE_2(ul) (uint8_t)((((uint32_t)ul) >> 16) & 0x000000FF)   /* IP address byte2 */
#define WIFI_ULONG_TO_IPV4BYTE_3(ul) (uint8_t)((((uint32_t)ul) >> 8)  & 0x000000FF)   /* IP address byte3 */
#define WIFI_ULONG_TO_IPV4BYTE_4(ul) (uint8_t)(((uint32_t)ul)         & 0x000000FF)   /* IP address byte4 */
...
#define WIFI_CFG_TASK_STACK_DEPTH       (1024)
#endif
#if defined(__CCRL__) || defined(__ICCRL78__) || defined(__RL)
#define WIFI_TASK_PRIORITY              (tskIDLE_PRIORITY + 0)  /* WiFi task priority */
#define WIFI_CFG_TASK_STACK_DEPTH       (configMINIMAL_STACK_SIZE * 20)
#endif
