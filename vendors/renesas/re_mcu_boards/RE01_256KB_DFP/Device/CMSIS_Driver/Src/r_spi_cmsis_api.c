...
#define ARM_SPI_PRV_DRV_VERSION         (ARM_DRIVER_VERSION_MAJOR_MINOR(1, 00)) /*!< driver version */
...
int32_t ARM_SPI_Initialize(ARM_SPI_SignalEvent_t cb_event, st_spi_resources_t * const p_spi) // @suppress("Non-API function naming")
{
    ...
    else
    {
        ...
        {
            (void)p_spi->tx_dma_drv->Close(); // @suppress("Cast comment")
        }
#endif
        ...
        {
            (void)p_spi->rx_dma_drv->Close(); // @suppress("Cast comment")
        }
#endif
    }
